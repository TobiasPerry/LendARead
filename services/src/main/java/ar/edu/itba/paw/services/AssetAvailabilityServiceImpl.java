...
public class AssetAvailabilityServiceImpl implements AssetAvailabilityService {
    ...
    public void returnAsset(final int lendingId) throws AssetInstanceNotFoundException, LendingCompletionUnsuccessfulException {
        ...
        emailService.sendReviewBorrower(lending.getAssetInstance(), lending.getUserReference(), lending.getId(), LocaleContextHolder.getLocale());
        emailService.sendReviewLender(lending.getAssetInstance(), lending.getAssetInstance().getOwner(), lending.getId(), LocaleContextHolder.getLocale());
    }
