...
public class AssetInstanceBorrowExceptionMapper  implements ExceptionMapper<AssetInstanceBorrowException> {
    ...
    public Response toResponse(AssetInstanceBorrowException e) {
        return Response.status(Response.Status.BAD_REQUEST).entity(ErrorDTO.fromError(messageSource.getMessage("exception.assetInstanceBorrowed", null, LocaleContextHolder.getLocale()),null)).build();
    }
}
