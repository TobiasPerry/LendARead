...
public class ImageNotFoundExceptionMapper implements ExceptionMapper<ImageNotFoundException> {
    ...
    public Response toResponse(ImageNotFoundException e) {
        return Response.status(Response.Status.BAD_REQUEST).entity(ErrorDTO.fromError(messageSource.getMessage("exception.imageNotFound", null, LocaleContextHolder.getLocale()),null)).build();
    }
}