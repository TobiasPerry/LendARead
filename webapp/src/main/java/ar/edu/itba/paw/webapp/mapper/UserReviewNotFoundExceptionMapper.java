...
public class UserReviewNotFoundExceptionMapper  implements ExceptionMapper<UserReviewNotFoundException> {
    ...
    public Response toResponse(UserReviewNotFoundException e) {
        return Response.status(Response.Status.NOT_FOUND).entity(ErrorDTO.fromError(messageSource.getMessage("exception.userReviewNotFound", null, LocaleContextHolder.getLocale()),null)).build();
    }
}
