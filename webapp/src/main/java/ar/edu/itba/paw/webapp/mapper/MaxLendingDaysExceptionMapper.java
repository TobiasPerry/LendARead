...
public class MaxLendingDaysExceptionMapper  implements ExceptionMapper<MaxLendingDaysException> {
    ...
    public Response toResponse(MaxLendingDaysException e) {
        return Response.status(Response.Status.BAD_REQUEST).entity(ErrorDTO.fromError(messageSource.getMessage("exception.maxLendingDays", null, LocaleContextHolder.getLocale()),null)).build();
    }
}