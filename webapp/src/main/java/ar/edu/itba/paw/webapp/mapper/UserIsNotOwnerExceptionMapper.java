...
public class UserIsNotOwnerExceptionMapper implements ExceptionMapper<UserIsNotOwnerException> {
    ...
    public Response toResponse(UserIsNotOwnerException e) {
        return Response.status(Response.Status.BAD_REQUEST).entity(ErrorDTO.fromError(messageSource.getMessage("exception.userIsNotOwner", null, LocaleContextHolder.getLocale()),null)).build();
    }
}
