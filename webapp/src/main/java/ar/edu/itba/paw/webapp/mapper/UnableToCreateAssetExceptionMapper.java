...
public class UnableToCreateAssetExceptionMapper implements ExceptionMapper<UnableToCreateAssetException> {
    ...
    public Response toResponse(UnableToCreateAssetException e) {
        return Response.status(Response.Status.BAD_REQUEST).entity(ErrorDTO.fromError(messageSource.getMessage("exception.unableToCreateAsset", null, LocaleContextHolder.getLocale()),null)).build();
    }
}
