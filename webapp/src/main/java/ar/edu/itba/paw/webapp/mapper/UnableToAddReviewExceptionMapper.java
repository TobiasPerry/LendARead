...
public class UnableToAddReviewExceptionMapper  implements ExceptionMapper<UnableToAddReviewException> {
    ...
    public Response toResponse(UnableToAddReviewException e) {
        return Response.status(Response.Status.BAD_REQUEST).entity(ErrorDTO.fromError(messageSource.getMessage("exception.unableToAddReview", null, LocaleContextHolder.getLocale()),null)).build();
    }
}
