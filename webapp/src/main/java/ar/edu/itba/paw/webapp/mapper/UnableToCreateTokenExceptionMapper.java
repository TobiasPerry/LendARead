...
public class UnableToCreateTokenExceptionMapper  implements ExceptionMapper<UnableToCreateTokenException> {
    ...
    public Response toResponse(UnableToCreateTokenException e) {
        return Response.status(Response.Status.BAD_REQUEST).entity(ErrorDTO.fromError(messageSource.getMessage("exception.unableToCreateToken", null, LocaleContextHolder.getLocale()),null)).build();
    }
}
