...
public class LocationNotExistExceptionMapper implements ExceptionMapper<LocationNotExistException> {
    ...
    public Response toResponse(LocationNotExistException e) {
        return Response.status(Response.Status.BAD_REQUEST).entity(ErrorDTO.fromError(messageSource.getMessage("exception.locationNotExist", null, LocaleContextHolder.getLocale()),null)).build();
    }
}
