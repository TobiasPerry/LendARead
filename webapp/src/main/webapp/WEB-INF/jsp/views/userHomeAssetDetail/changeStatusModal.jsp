...
<div class="modal fade" id="changeStatusModal" tabindex="-1" role="dialog" aria-labelledby="${param.modalTitle}" aria-hidden="true">
      ...
      <div class="modal-footer border-0">
        ...
        <form action="${changeStatusUrl}" method="post">
          <button type="submit" class="btn btn-primary rounded-pill px-4 py-2" style="background-color: #2B3B2B; border-color: #00B4A0;">
            <c:if test="${asset.assetState.isPrivate()}">
              <spring:message code="userHomeView.changeVisibityPublic.button" />
            </c:if>
            <c:if test="${asset.assetState.isPublic()}">
              <spring:message code="userHomeView.changeVisibityPrivate.button" />
            </c:if>
          </button>
        </form>
      </div>
